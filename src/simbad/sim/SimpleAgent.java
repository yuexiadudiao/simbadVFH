...
public  class SimpleAgent extends BaseObject {
    ...
    //【遥操作】
    protected void runTeleoperation(){}
   ...
   protected void dispose(){
	...
	//【保存轨迹的函数】
	public void saveTrajectory(int fps)
	{
    		if(getCounter()%fps==0)//采样频率0.5s一次
        	{
        		Point3d coords = new Point3d();
        		getCoords(coords);
            	System.out.println(coords.toString());
       			File file = new File("d:/trajectory.txt");  
    	        try  
    	        {  
    	            FileWriter fw = new FileWriter(file,true);  
    	            BufferedWriter bw = new BufferedWriter (fw);//以追加的方式写文件
    	           		            
    	            	fw.write(Double.toString(coords.x)); 
    	            	fw.write(" ");
    	            	fw.write(Double.toString(coords.z)); 
    	            	fw.write("\n");
